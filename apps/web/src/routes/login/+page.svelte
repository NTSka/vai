    ...
    try {
      ...
      errorMessage =
        error instanceof ApiError && error.status === 401
          ...
          : "Login failed. Check backend availability and try again.";
    } finally {
      submitting = false;
    }
  }
...
<main class="grid min-h-screen place-items-center bg-panel px-4 py-8">
  <form class="panel w-full max-w-sm p-5" on:submit|preventDefault={submit}>
    <div class="mb-5">
      ...
      <p class="mt-1 text-sm text-slate-600">Sign in to the MVP workspace.</p>
    </div>
    ...
    <label class="label mt-4 block" for="password">Password</label>
    <input
      id="password"
      class="field mt-1"
      type="password"
